...
def login_details(username,password):
    file=open("public_login_details.txt","w+")
...
class Home():
    ...
    def __init__(self,master):
        ...
        btn2 = Button(frame, text='Change Password', background="Brown", fg="White", command=self.change)
        btn2.pack(fill = X)
        frame.pack(padx=100, pady=19)
        
        btn0 = Button(frame, text='Sign Up', background="Brown", fg="White", command=self.sign_up)
        btn0.pack(fill = X)
        frame.pack(padx=100, pady=19)
    ...
    def login_submit(self):
        ...
        try:
            ...
            rows = session.execute('SELECT username,password FROM public_record')
            for row in rows:
